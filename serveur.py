...
class Partie:
    ...
    async def lancer_chrono(self):
        ...
        try:
            ...
            # Incrémenter AFK AVANT le sleep — après le sleep chrono_task peut avoir changé
...
async def traiter_langue_au_chat(partie: Partie, demandeur_id: str):
    ...
    if partie.joueur_fautif and partie.joueur_fautif in partie.joueurs:
        fautif = partie.joueurs[partie.joueur_fautif]
        if fautif.en_vie:
            interpelle_id = partie.joueur_fautif
        else:
            # Fautif éliminé — chercher le dernier joueur vivant avant le joueur actuel
            idx = partie.ordre.index(demandeur_id) if demandeur_id in partie.ordre else 0
            for i in range(1, len(partie.ordre)):
                cid = partie.ordre[(idx - i) % len(partie.ordre)]
                if cid != demandeur_id and partie.joueurs.get(cid) and partie.joueurs[cid].en_vie:
                    interpelle_id = cid
                    break
            else:
                await manager.envoyer(partie.room_id, demandeur_id, {"type": "erreur", "message": "Personne à interpeller !"})
                return
    else:
        # Pas de fautif enregistré — prendre le joueur vivant précédent
        idx = partie.ordre.index(demandeur_id) if demandeur_id in partie.ordre else 0
        interpelle_id = None
        for i in range(1, len(partie.ordre)):
            cid = partie.ordre[(idx - i) % len(partie.ordre)]
    ...
    try:
        ...
        await manager.diffuser(partie.room_id, {
            **partie.snapshot(),
            ...
            "valide": False,
            "interpelle": interpelle_id,
            "message": f"⏰ {partie.joueurs.get(interpelle_id, EtatJoueur(id='',nom='?',vies=0)).nom} n'a pas répondu à temps !",
        })
        await asyncio.sleep(1.5)
...
async def traiter_stopper_sequence(partie: Partie, joueur_id: str):
    """Le joueur actuel refuse de continuer — force la complétion du pays en cours."""
    if partie.etat != EtatPartie.EN_COURS:
        return
    if joueur_id != partie.joueur_actuel_id:
        await manager.envoyer(partie.room_id, joueur_id, {"type": "erreur", "message": "Ce n'est pas ton tour !"})
    ...
    if not pays_exact:
        ...
        return
    if pays_exact["nom_normalise"] in partie.pays_joues_noms:
        await manager.envoyer(partie.room_id, joueur_id, {"type": "erreur", "message": f"{pays_exact['nom']} a déjà été joué !"})
        return
