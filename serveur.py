...
def normaliser(s: str) -> str:
    """Supprime les accents pour la recherche clavier."""
...
async def demarrer_tour(partie: Partie):
...
async def traiter_lettre(partie: Partie, joueur_id: str, lettre: str):
    ...
    if not possibilites:
        ...
        # On informe tout le monde mais on ne retire pas encore de vie
        ...
        # On passe quand même au joueur suivant pour qu'il puisse réagir
...
async def traiter_reponse_langue_au_chat(partie: Partie, joueur_id: str, pays_propose: str):
    ...
    match = next((p for p in get_pays(partie.config.langue) if p["nom_normalise"] == nom_norm), None)
    ...
    elif not match["nom_normalise"].startswith(normaliser(partie.sequence)):
...
async def ia_jouer(partie: Partie):
    ...
    seq = partie.sequence
    possibilites = chercher_pays(seq, partie.config.langue, partie.config.mode_mixte)

    if not possibilites:
        # L'IA est piégée → elle donne sa langue au chat
        # Elle révèle une séquence de la manche précédente (bluff)
        await manager.diffuser(partie.room_id, {
            "type": "ia_langue_au_chat",
            "message": "🤖 L'IA donne sa langue au chat ! Elle perd une vie.",
        })
        await partie.appliquer_perte_vie(partie.joueur_actuel_id, "IA piégée")
        return

    # Stratégie : l'IA choisit un pays au hasard parmi les possibilités
    # et pose la lettre suivante — elle évite volontairement de compléter le mot
    # sauf si elle n'a pas le choix
    cibles = [p for p in possibilites if len(p["nom_normalise"]) > len(seq) + 1]
    if not cibles:
        cibles = possibilites  # forcée de risquer
    ...
    lettre_suivante = nom_cible[len(seq)]
