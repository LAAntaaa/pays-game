...
class Partie:
    ...
    async def lancer_chrono(self):
        ...
        # Capturer le joueur actuel AU DÉBUT — self.joueur_actuel_id peut changer
        jid = self.joueur_actuel_id
        try:
            await asyncio.sleep(self.config.temps)
            # Vérifier que c'est encore le même joueur (sinon le chrono a été annulé)
...
async def traiter_lettre(partie: Partie, joueur_id: str, lettre: str):
    ...
    if joueur_id != partie.joueur_actuel_id:
        ...
        return
    # NOTE : on ne bloque plus si en_attente_langue_au_chat — les deux timers sont indépendants
    ...
    if not possibilites:
        # Séquence invalide → le joueur a joué (reset AFK), le suivant peut demander langue au chat
        import time
    ...
    # Séquence valide → joueur a bien joué, reset AFK
    partie.tours_sans_jouer[joueur_id] = 0
    partie.sequence = nouvelle_seq
    partie.annuler_chrono()
...
async def traiter_langue_au_chat(partie: Partie, demandeur_id: str):
    ...
    # Donner sa langue au chat est une action → réinitialiser le compteur AFK
    partie.tours_sans_jouer[demandeur_id] = 0
    ...
    # NE PAS annuler le chrono — le jeu continue, l'interpellé a son propre timer
...
async def _timeout_langue_au_chat(partie: Partie, interpelle_id: str, delai: int):
    ...
    Le jeu continue indépendamment — ce timer tourne en parallèle du chrono principal."""
    try:
        await asyncio.sleep(delai)
        # Vérifier que la situation n'a pas changé (réponse reçue entre-temps)
        ...
        await manager.diffuser(partie.room_id, {
            **partie.snapshot(),
            ...
            "interpelle": interpelle_id,
            "timeout": True,
            "message": f"⏰ {partie.joueurs.get(interpelle_id, type('o', (), {'nom': '?'})()).nom} n'a pas répondu — perd une vie !",
        })
        await asyncio.sleep(0.5)
        await appliquer_perte_vie_externe(partie, interpelle_id, "Timeout langue au chat")
    except asyncio.CancelledError:
        pass
...
async def traiter_reponse_langue_au_chat(partie: Partie, joueur_id: str, pays_propose: str):
    """Le joueur interpellé révèle son pays. Le jeu continue indépendamment."""
    ...
    # Annuler le timeout et libérer le flag IMMÉDIATEMENT
    if partie.lac_timeout_task and not partie.lac_timeout_task.done():
        partie.lac_timeout_task.cancel()
    ...
    if match is None:
        ...
        # Pays valide → le DEMANDEUR (joueur actuel) perd une vie
...
async def traiter_stopper_sequence(partie: Partie, joueur_id: str):
    """Le joueur actuel choisit de NE PAS continuer la séquence Niger-like.
    → L'auteur de la dernière lettre a de facto complété un pays, il perd une vie."""
    if partie.etat != EtatPartie.EN_COURS:
        return
    if joueur_id != partie.joueur_actuel_id:
        await manager.envoyer(partie.room_id, joueur_id, {"type": "erreur", "message": "Ce n'est pas ton tour !"})
    ...
    if not pays_exact:
        await manager.envoyer(partie.room_id, joueur_id, {"type": "erreur", "message": "La séquence actuelle n'est pas un pays !"})
        return

    # L'auteur de la séquence = joueur_fautif (celui qui a posé la dernière lettre)
    fautif_id = partie.joueur_fautif or joueur_id
    if fautif_id not in partie.joueurs:
        fautif_id = joueur_id

    # Enregistrer le pays comme complété
    ...
    if not partie.joueurs.get(partie.joueur_actuel_id, EtatJoueur(id="", nom="", vies=0)).est_ia:
        return
    # Ne pas jouer si une langue au chat est en attente — laisser _ia_repondre ou le joueur humain gérer
    ...
    try:
        while True:
            ...
            if action == "lettre":
                ...
                # Le joueur actuel refuse de continuer la séquence Niger-like
                # → l'auteur de la dernière lettre a forcé un pays complet, il perd une vie
